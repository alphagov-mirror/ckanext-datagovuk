...
_trim_strings_for_index_DEFAULT_LIMIT = object()  # sentinel value
def trim_strings_for_index(
    data_dict,
    limit=_trim_strings_for_index_DEFAULT_LIMIT,
    safe_key_patterns=_index_long_string_safe_key_patterns,
):
    """
    return a copy of @data_dict, with any string values truncated to length @limit unless
    they are under a toplevel key matching one of @safe_key_patterns.
    """
    if limit is trim_strings_for_index.DEFAULT_LIMIT:
        limit = config.get(
...
trim_strings_for_index.DEFAULT_LIMIT = _trim_strings_for_index_DEFAULT_LIMIT
